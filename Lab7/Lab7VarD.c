...
void RemoveElement(int **array, int *size, int value) {
    int write_ind = 0;

    for (int ind = 0; ind < *size; ind++) {
        if ((*array)[ind] != value) {
            (*array)[write_ind++] = (*array)[ind];
        }
    }

    *size = write_ind;

    if (*size > 0) *array = (int*)realloc(*array, (*size) * sizeof(int));
    else {
        free(*array);
        *array = NULL;
    }
...
void UpdateArray(int **array, int *size) {
    ...
    *size = end_ind - start_ind + 1;
    ...
    for (int ind = start_ind; ind <= end_ind; ind++) {
        if ((*array)[ind] % 2 == 0) RemoveElement(array, size, (*array)[ind]);
    }
}
